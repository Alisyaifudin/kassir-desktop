...
import { DefaultError, err, logOld, NotFound, ok, ResultOld, tryResult } from "../../lib/utils";
...
async function getCount(tx: Database): Promise<ResultOld<DefaultError, number>> {
...
function get(tx: Database) {
  return {
    async all(): Promise<ResultOld<"Aplikasi bermasalah", { id: number; mode: TX.Mode }[]>> {
      return tryResult({
        run: () =>
          tx.select<{ id: number; mode: TX.Mode }[]>("SELECT name, mode FROM transactions"),
      });
    },
    async byId(id: number): Promise<ResultOld<"Aplikasi bermasalah" | NotFound, TX.Transaction>> {
      const [errMsg, res] = await tryResult({
...
function add(tx: Database) {
  return {
    async one(): Promise<ResultOld<DefaultError | "Terlalu banyak", number>> {
