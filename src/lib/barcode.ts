import { err, logOld, ok, ResultOld } from "./utils";
...
export function ean13(num: number): ResultOld<"Format salah", number> {
  const nums = toArray(num);
  if (nums.length !== 12) {
    logOld.error("Jumlah digit bukan 12: " + nums.length.toString());
