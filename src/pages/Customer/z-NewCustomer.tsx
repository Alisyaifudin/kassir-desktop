...
export function NewCustomer({ onAdd }: Props) {
  ...
  const form = useForm({
    ...
    async onSubmit({ value }) {
      const errMsg = await onAdd(value.name, value.phone);
      setError(errMsg);
      if (errMsg === null) {
        setOpen(false);
        form.reset();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Tambahkan data pelanggan baru beserta nomor kontak.</DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault();
