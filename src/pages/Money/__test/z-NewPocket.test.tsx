...
describe("NewPocket", () => {
  ...
  test("opens dialog when trigger is clicked", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /tambah kantong keuangan baru/i }),
    ).not.toBeNull();
  });
  ...
  test("shows validation error when name is empty", async () => {
    ...
    expect(
      await screen.findByText(/tidak boleh kosong/i),
    ).not.toBeNull();
  });
  ...
  test("shows validation error when name exceeds 20 chars", async () => {
    ...
    expect(
      await screen.findByText(/maksimal 20 karakter/i),
    ).not.toBeNull();
  });
