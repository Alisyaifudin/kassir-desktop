...
export const deletePocket = Effect.gen(function* () {
  const moneyService = yield* MoneyService;
  const onDelete = (pocketId: string) => moneyService.pocket.delete(pocketId);
  return function DeletePocketBtn({ pocketId }: { pocketId: string }) {
    const [open, setOpen] = useState(false);
    const { loading, error, handleDelete } = useDeletePocket(onDelete, pocketId, () =>
      setOpen(false),
    );
    return (
...
function useDeletePocket(
  onDelete: (kindId: string) => Promise<string | null>,
  kindId: string,
  onClose: () => void,
) {
  ...
  async function handleDelete() {
    setLoading(true);
    const errMsg = await onDelete(kindId);
    setLoading(false);
    setError(errMsg);
    if (errMsg === null) {
      onClose();
