...
function UploadedEntries({
  ...
  pocketId: string;
  items: MoneyImport[];
  add: (pocketId: string, record: MoneyImport) => Promise<MoneyUploadError | null>;
  onDone: () => void;
}) {
  const progress = useUploadProgress({
    items,
    getId: (m) => String(m.timestamp),
    add: (m) => add(pocketId, m),
    onDone,
  });
    ...
    <div className="space-y-2">
      <div className="w-full bg-secondary rounded-full h-2 overflow-hidden">
        <div
          ...
          style={{
            width: `${(progress.filter((p) => p.state !== "pending").length / items.length) * 100}%`,
          }}
        />
      </div>
      <p className="text-small text-muted-foreground">
        {progress.filter((p) => p.state !== "pending").length}/{items.length} data tersimpan
      </p>
      <ol className="mt-4 space-y-2 max-h-48 overflow-y-auto">
        {progress.map((item, i) => {
          if (item.state === "pending" && i !== firstPendingIndex) return null;
          return (
            <li key={item.item.timestamp} className="flex flex-col gap-1 text-small">
              <div className="flex items-center gap-2">
                ...
                <span>{formatEpochtime(item.item.timestamp, { date: "long", time: "long" })}</span>
              </div>
...
function ErrorComp({ error }: { error: MoneyUploadError }) {
  switch (error._tag) {
    case "DuplicateError":
      return <span className="text-amber-600 truncate">Sudah ada — dilewati</span>;
    case "MoneyError":
