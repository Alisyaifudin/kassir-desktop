...
import { extractMoney } from "./util-validate-money";
import { uploadSelected } from "./effect-uploadSelected";
...
export const uploadMoney = Effect.gen(function* () {
  const UploadSelected = yield* uploadSelected;
  return function UploadMoney({ pocketId }: { pocketId: string }) {
    return (
      <Dialog>
        ...
        <DialogContent className="flex overflow-y-auto max-h-[calc(100vh-2rem)] max-w-2xl flex-col p-0">
          ...
          <div className="min-h-0 flex-1 px-6 pb-6">
            <UploadInput extract={extractMoney}>
              {(data) => <UploadSelected pocketId={pocketId} data={data} onRemove={() => {}} />}
            </UploadInput>
          </div>
        </DialogContent>
      </Dialog>
