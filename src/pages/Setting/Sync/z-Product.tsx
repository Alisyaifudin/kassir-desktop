  ...
  return (
    <li className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2">
        {count.server > 0 ? <span>Unduh Data: {count.server}</span> : null}
        {count.unsync > 0 ? <span>Belum Tersinkron: {count.unsync}</span> : null}
      </div>
...
function useSync(token: string) {
  ...
  const { error, handler, loading } = useCallEffect(() => program(token, setCount));
