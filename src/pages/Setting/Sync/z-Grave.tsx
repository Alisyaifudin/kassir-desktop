...
export function GraveSync({ token, icon: Icon }: { token: string; icon: LucideIcon }) {
  ...
  return (
    <li className="flex flex-col gap-1.5 px-4 py-3 hover:bg-muted/30 transition-colors">
      <div className="flex items-center gap-2">
        ...
        <span className="text-normal font-medium flex-1">Sampah</span>
...
function useSync(token: string) {
  ...
  const { error, handler, loading } = useCallEffect(() => programSyncGrave(token, setCount));
  return { error, handleClick: handler, loading, count };
}

const LIMIT = 1000;

function programSyncGrave(
  token: string,
  ...
  return Effect.gen(function* () {
    ...
    for (let i = 0; i < LIMIT; i++) {
      const count = yield* sync.grave(token, stop);
