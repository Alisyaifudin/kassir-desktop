...
import { extractProduct, MAXIMUM_SIZE, ProductImport } from "./util-validate-product";
...
export function PorductUpload() {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dragDepthRef = useRef(0);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [data, setData] = useState<{ name: string; products: ProductImport[] } | undefined>(
    undefined,
  );
  ...
  const processFiles = async (fileList: FileList | null) => {
    ...
    setLoading(true);
    const either = await Effect.runPromise(Effect.either(extractProduct(nextFile)));
    setLoading(false);
    dragDepthRef.current = 0;
    setIsDragActive(false);
    Either.match(either, {
