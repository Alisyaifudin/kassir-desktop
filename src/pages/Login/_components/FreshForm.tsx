...
export function FreshForm({context, revalidate}: {revalidate: ()=> void, context: {db: Database, store: Store}}) {
	const navigate = useNavigate();
	const { handleSubmit, error, loading } = useFresh(navigate, revalidate, context);
	return (
		<div className="flex flex-col gap-5 p-5 bg-white mx-auto w-full max-w-5xl ">
			...
			<form onSubmit={handleSubmit} className="flex flex-col gap-2 text-3xl">
				...
				<label className="grid items-center grid-cols-[250px_1fr]">
					...
					<Input name="confirmed-password" type="password" aria-autocomplete="list" />
				</label>
