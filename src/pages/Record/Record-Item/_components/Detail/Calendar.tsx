					...
					<DialogHeader>
						<div className="flex flex-row items-center gap-2">
							...
							<Button className="w-fit" variant="secondary" onClick={() => changeMode("year")}>
								<DialogTitle className="text-3xl">{startOfDate.year}</DialogTitle>
							</Button>
							<Button onClick={handleNext}>
								<ChevronRight size={35} />
							</Button>
	...
	return (
		<DialogContent>
			...
			<form onSubmit={handleSubmitYear}>
				<Input type="number" defaultValue={startOfDate.year} name="year" id="year-input" aria-autocomplete="list" />
			</form>
			<DialogFooter className="flex items-center justify-between">
				<Button
					variant="outline"
