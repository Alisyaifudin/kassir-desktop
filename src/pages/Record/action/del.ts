...
import { DefaultError, integer, logOld } from "~/lib/utils";
...
export async function delAction(formdata: FormData, href: string) {
  ...
  if (errMode !== null) {
    logOld.error("Failed to get record mode");
    return errMode;
  }
  const [errProd, products] = await db.recordProduct.get.product(timestamp);
  if (errProd !== null) {
    logOld.error("Failed to get record products with product id");
    return errProd;
  }
  ...
  for (const errMsg of res) {
    if (errMsg !== null) {
      logOld.error("Failed to update stock");
      return errMsg;
    }
  }
  const errMsg = await db.record.delByTimestamp(timestamp);
  if (errMsg !== null) {
    logOld.error("Failed to delete record");
    return errMsg;
  }
