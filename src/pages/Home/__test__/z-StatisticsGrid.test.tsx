...
describe("StatisticsGrid", () => {
  ...
  function renderGrid(opts?: {
    ...
    expenseLoader?: () => Effect.Effect<
      ...
      DailySummaryError
    >;
    totalLoader?: () => Effect.Effect<{ in: number; out: number }, DailySummaryError>;
  }) {
    return render(
      <StatisticsGrid
  ...
  describe("error state", () => {
    test("shows error for income card when loader fails", async () => {
      renderGrid({
        incomeLoader: () => Effect.fail(new DailySummaryError(new Error("Koneksi gagal"))),
      });
    ...
    test("shows error for expense card when loader fails", async () => {
      renderGrid({
        expenseLoader: () => Effect.fail(new DailySummaryError(new Error("Timeout"))),
      });
    ...
    test("shows error for total transactions when loader fails", async () => {
      renderGrid({
        totalLoader: () => Effect.fail(new DailySummaryError(new Error("Server down"))),
      });
    ...
    test("partial failure — expense fails while others succeed", async () => {
      renderGrid({
        expenseLoader: () => Effect.fail(new DailySummaryError(new Error("Parsial"))),
      });
