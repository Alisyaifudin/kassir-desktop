...
export function getByRecordIds(ids: string[]) {
  const placeholder = ids.map((_, i) => `$${i + 1}`).join(", ");
  return Effect.gen(function* () {
    const rows = yield* DB.try((db) =>
      db.select<Output[]>(
        `SELECT record_id, product_id, record_products.record_product_id, record_product_name, 
         ...
         WHERE record_id IN (${placeholder})
