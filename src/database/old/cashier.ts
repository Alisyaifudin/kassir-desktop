...
function get(db: Database) {
  return {
    ...
    async hash(
      ...
      Result<"Aplikasi bermasalah" | "Kasir tidak ditemukan", { hash: string; role: DB.Role }>
    > {
      const [errMsg, res] = await tryResult({
        run: () =>
          db.select<{ hash: string; role: DB.Role }[]>(
            "SELECT hash, role FROM cashiers WHERE name = $1",
            [name],
          ),
      });
