...
let cache: null | DB.Product[] = null;
...
export class ProductTable {
  ...
  // private _getInstance: ReturnType<typeof get> | null = null;
  // get get() {
  // 	if (!this._getInstance) {
  // 		this._getInstance = get(this);
  // 	}
...
function get(self: ProductTable) {
  return {
    ...
    async byId(id: number): Promise<Result<DefaultError | NotFound, DB.Product>> {
      const [errMsg, products] = await tryResult({
        run: () => self.db.select<DB.Product[]>("SELECT * FROM products WHERE id = $1", [id]),
      });
      if (errMsg !== null) return err(errMsg);
      if (products.length === 0) return err("Tidak ditemukan");
  ...
  return {
    async one(data: {
      ...
      try {
        ...
        // update caches
        if (cache) {
          const productIndex = cache.findIndex((p) => p.id === data.id);
          cache[productIndex] = {
            id: cache[productIndex].id,
            name: data.name.trim(),
            stock: data.stock,
