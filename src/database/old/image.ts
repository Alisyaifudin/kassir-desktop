...
function del(db: Database) {
  return {
    async byId(id: number): Promise<Result<"Aplikasi bermasalah" | "Gambar tidak ada", string>> {
      const [errSelect, res] = await tryResult({
        run: () => db.select<{ name: string }[]>("SELECT name FROM images WHERE id = $1", [id]),
      });
      if (errSelect) return err(errSelect);
      if (res.length === 0) return err("Gambar tidak ada");
