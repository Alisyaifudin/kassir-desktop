...
export function getPocketById(pocketId: string, start: number, end: number) {
  return Effect.gen(function* () {
    const [pocket, money] = yield* Effect.all([
      sqlx.pocket.get.byId(pocketId),
      getMoneyByRange(pocketId, start, end),
    ]);
    return {
      ...pocket,
      money,
...
function getMoneyByRange(pocketId: string, start: number, end: number) {
  return Effect.gen(function* () {
